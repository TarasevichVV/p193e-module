                    ...
                    stage('Sab2') {
                        container('centos') {
                            ...
                            sh """
                                ...
                                kubectl patch deployment shanchar-deploy -n shanchar -p '{"spec": {"template": {"spec": {"containers": [{"name": "shanchar-deploy", image: "nexus-dock.k8s.playpit.by:80/helloworld-shanchar:$BUILD_NUMBER"}]}}}}'
                                sleep 5
                                kubectl get ns
